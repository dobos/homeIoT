...
srv:listen(80,function(conn) 
  ...
  conn:on("receive",function(conn,payload) 
    ...
    --if vars=="edit" then
		...
		--conn:send("x.onreadystatechange=function(){if(x.readyState==4) tag(x.responseText);};")
		--conn:send("x.send(new Blob([document.getElementsByName('t')[0].value],{type:'text/plain'}));\">Save</button>")
		--conn:send("<a href='?run'>run</a><w></w>")
    --end    
